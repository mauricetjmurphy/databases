...
import (
	...
	"github.com/NBCUniversal/gvs-ms-common/clients/aws/secrets"
	"github.com/NBCUniversal/gvs-ms-common/db/migrate"
	"github.com/NBCUniversal/gvs-ms-common/db/query"
...
func (db *dbImpl) Update(ctx context.Context, value interface{}) error {
	...
	if result := db.Session(&gorm.Session{FullSaveAssociations: true}).Updates(value); result.Error != nil {
		return result.Error
	}
	return nil
}
